class Bruch
{
    ...
    public Bruch(string bruchtext)  //konstruktor ist wenn es die gleichen Namen wie die klasse hat
    {
        ...
        try
        {
            // Prüfen ob es eine gemischte Zahl ist (z.B. "5 3/4")
            if (bruchtext.Contains(" ") && bruchtext.Contains("/"))
            {
                string[] teile = bruchtext.Split(' ', System.StringSplitOptions.RemoveEmptyEntries);
                if (teile.Length != 2) throw new System.FormatException("Ungültiges Format für gemischte Zahl.");
                
                int ganz = int.Parse(teile[0].Trim());
                string[] bruchTeile = teile[1].Split('/');
                if (bruchTeile.Length != 2) throw new System.FormatException("Ungültiges Bruchformat.");
                
                int z = int.Parse(bruchTeile[0].Trim());
                int n = int.Parse(bruchTeile[1].Trim());
                if (n == 0) throw new System.ArgumentException("Nenner darf nicht 0 sein.");
                
                // Gemischte Zahl in unechten Bruch umwandeln
                // z.B. 5 3/4 = (5*4 + 3)/4 = 23/4
                int neuerZaehler = ganz * n + z;
                InitAndReduce(neuerZaehler, n);
            }
        ...
        {
            throw new System.FormatException("Bruch konnte nicht gelesen werden. Erwartet zum Beispiel '3/4', '2' oder '5 3/4'.");
        }
    }
